...
#邮件
content="请尽快确认重启结果,服务器IP：10.138.12.130 [$(date +'%F %H:%M:%S')]"
title="HAC1169-V2.2服务出现重启"
Email='389554843@qq.com,572512893@qq.com,690948092@qq.com'

#获取dubbo服务PID
DubboID=`ps aux |grep hac1169-service-impl-2.2.0-SNAPSHOT |grep -v grep|awk '{print $2}'`

OKCODE=200
# dubbo_start.sh
StartDubbo=/home/hac/deploy/hac2.2/hac1169-service-impl-2.2.0-SNAPSHOT/bin/start.sh

# 定义要监控的页面地址
WebUrl=http://10.135.12.130:1026/com.haier.openplatform.hac.service.HacUserServiceCli?wsdl

# 获取返回状态码
DubboServiceCode=`curl -s -i -X  GET --head -m 3 $WebUrl |awk 'NR==1{print $2}'`
# 日志输出
DubboMonitorLog=/home/hac/script/DubboMonitor.log

Monitor()
{
  echo "[info]开始监控HAC1169-DubboV2.2...[$(date +'%F %H:%M:%S')]"
