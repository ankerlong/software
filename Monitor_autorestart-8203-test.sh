...
#邮件
content="请尽快确认重启结果,服务器IP：10.138.8.203 [$(date +'%F %H:%M:%S')]"
title="HAC1169-V2.1 TEST服务出现重启"
...
OKCODE=200
# dubbo_start.sh
StartDubbo=/home/textport/deploy/hac1169-service-impl-2.1.0-SNAPSHOT/bin/start.sh

# 定义要监控的页面地址
WebUrl=http://10.138.8.203:1024/com.haier.openplatform.hac.service.HacUserServiceCli?wsdl

# 获取返回状态码
DubboServiceCode=`curl -s -i -X  GET --head -m 3 $WebUrl |awk 'NR==1{print $2}'`
# 日志输出
DubboMonitorLog=/home/textport/script/DubboMonitor.log
